...
import { UserInstance, Match } from 'models';
...
export default ({ label, dispatch, match }: UserProps) => {
  const { name, avatar_url, email } = useAppSelector((user: UserInstance) => user);

  const [ url ] = useState(`/users/${match.params.userName}`);
  const [ isLoading, setIsLoading ] = useState(true);

  useEffect(() => {
    api.users.getUser(url,
        (r) => {
      ...
      ><CloseIcon/></span>
        <div className={`${label}__block`}>
          ...
          <div className={`${label}__info`}>
            <h3 >{name}</h3>
            <p className={`${label}__email`}>{email}</p>
          </div>
        </div>
      </Section>
    </>
